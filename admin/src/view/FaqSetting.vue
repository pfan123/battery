<template>
  <div class="sys_setting">
    <h2 class="sys_setting_title g_cf">{{title}} <el-button class="news_sendbtn" type="primary" @click="onLinkEdit">发布疑难问题</el-button></h2>
    <el-table
      :data="tableData"
      border
...
export default {
  ...
  methods: {
    onLinkEdit(){
      this.$router.push({ path: `/dashboard/faqedit/create` })
    },
    ...
    dialogConfirm(){
      ...
      if(faqId && type == "edit")this.$router.push({ path: `/dashboard/faqedit/${faqId}` })
      if(faqId && type == "dete"){
        this.$http.post('/api/dashboard/deteFaq.json', { id: faqId }).then( ({ data }) => {
          if(data.success){
  ...
  created(){
      this.$http.get('/api/dashboard/getFaqList.json').then( ({ data }) => {
        if(data.success){
          let json = data.data
